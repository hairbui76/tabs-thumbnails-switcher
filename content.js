(() => {
  ...
  function buildOverlay(tabs, activeTabId) {
    ...
    settingsBtn.className = "tts-header-settings";
    ...
    tabs.forEach((tab, i) => {
      ...
      if (tab.id === activeTabId) card.classList.add("tts-current");
    ...
    hint.innerHTML = OK.hint(overlayKeyState);
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === "show-switcher") {
      ...
      buildOverlay(message.tabs, message.activeTabId);
    }
  });
})();
